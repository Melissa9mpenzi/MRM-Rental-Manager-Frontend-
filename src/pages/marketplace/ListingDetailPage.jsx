...
import { MapPin, MessageCircle, Calendar, Wifi, Car, Bath, BedDouble, Maximize2, Lock, CheckCircle2, Wind, Shield, Zap } from "lucide-react";
import { getPublicListing } from "../../data/publicListings";
...
const FALLBACK = {
  ...
  desc: "Modern finishes, high ceilings, and a balcony. Schedule a viewing to experience the space.",
  parking: "2 slots",
};
...
export default function ListingDetailPage() {
  ...
  const p = getPublicListing(id) || FALLBACK;
  const gallery = [p.image, "/images/listing-interior.jpg", "/images/listing-luxury.jpg"].filter(Boolean);
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_340px]">
        <div className="space-y-4">
          <div className="overflow-hidden rounded-3xl border border-white/[0.1] bg-[#0a1018]">
            ...
            {p.verified && (
              <div className="absolute left-4 top-4 z-10 inline-flex items-center gap-1.5 rounded-full border border-[#00C896]/35 bg-black/55 px-3 py-1 text-[11px] font-extrabold uppercase tracking-wide text-[#00C896] backdrop-blur-md">
                <CheckCircle2 size={14} /> Verified property
              </div>
            )}
              {isAuthed && user?.role === "tenant" && (
                <div className="absolute right-3 top-3 z-10">
                  <ListingSaveButton listingId={Number(id)} compact />
                </div>
              )}
            </div>
          </div>
          ...
          <div className="card-glass p-6">
            ...
            <div className="mt-6 flex items-center gap-2 rounded-xl border border-white/[0.08] bg-white/[0.04] px-4 py-3 text-sm text-white/55">
              ...
              <span>Fiber-ready building — confirm speeds with the landlord before you move in.</span>
            </div>
            <div className="mt-8">
        ...
        <div className="card-glass h-fit space-y-4 p-6 lg:sticky lg:top-4">
          <div>
            ...
            <div className="text-3xl font-extrabold text-[#00C896]">UGX {p.price.toLocaleString()}</div>
            <div className="text-xs text-white/40">per month · negotiable</div>
            <p className="mt-2 text-[11px] font-semibold uppercase tracking-wide text-white/35">Apartment</p>
          </div>
          ...
          {!isAuthed && (
            <div className="flex items-start gap-2 rounded-xl border border-amber-500/25 bg-amber-500/10 px-3 py-2.5 text-xs text-amber-100/90">
              ...
              <span>
                You can view this listing without an account. Sign in to message the landlord, book a viewing, or use
                rent tools.
              </span>
            </div>
          )}
          ...
          {isAuthed ? (
            <>
              ...
              <ListingSaveButton listingId={Number(id)} />
            </>
          ) : (
            <>
